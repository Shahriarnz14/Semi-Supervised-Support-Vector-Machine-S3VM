...
if __name__ == "__main__":
    ...
    for ii in range(10):
        for jj in range(ii+1, 10):
            str_path = "../MNIST_qs3vm/MNIST_qs3vm_" + str(ii)+str(jj)+"_.dat"
            #print str(ii)+str(jj)
            # X_train_l, L_train_l, X_train_u, X_test, L_test = examples.get_gaussian_data(my_random_generator, str_path)
            # t_start = time.time()
    ...
    X_train_l, L_train_l, X_train_u, X_test, L_test = examples.get_gaussian_data(my_random_generator,"../MNIST_qs3vm/MNIST_qs3vm_49_.dat")
    t_start = time.time()
    model = QN_S3VM(X_train_l, L_train_l, X_train_u, my_random_generator, lam=1, lamU=1)
    model.train()
    t_end = time.time()
